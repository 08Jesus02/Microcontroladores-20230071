/**
 * @file main.c
 * @brief Portón automático con ESP32 + ESP-IDF + MQTT (+ Portal WiFi/MQTT)
 * @version 1.0
 *
 * @details
 * Sistema de control de portón con:
 *  - Máquina de estados (FSM) con bucles por estado.
 *  - Conectividad WiFi: Modo AP de configuración y modo STA de operación.
 *  - Portal web de configuración (HTTP GET/POST), sin defaults: WiFi y MQTT.
 *  - Persistencia en NVS (SSID, password, URI MQTT, tópicos, modo de arranque).
 *  - Timeout de conexión de 30s (si no se obtiene IP → reinicia en modo AP).
 *  - MQTT dinámico (se reinicia al cambiar parámetros), tópicos configurables.
 *  - Interlock en relés, antirrebote, timeouts de movimiento y telemetría periódica.
 *
 * Cambios de esta versión:
 *  - Portal WiFi/MQTT con NVS (sin defaults), formularios separados (act=wifi / act=mqtt).
 *  - Wipe para volver a AP (borra credenciales y restaura modo configuración).
 *  - Timeout 30s sin IP ⇒ reinicia en AP. AP se apaga al obtener IP STA.
 *  - url_decode_inplace para application/x-www-form-urlencoded (+ POST para evitar 431).
 */
...
// Notas: GPIO34/35 no tienen pulls internos → usar resistencias externas si LS son NC.
#define PIN_LSC        GPIO_NUM_35   ///< Fin de carrera CERRADO (entrada sin pull interno)
#define PIN_LSA        GPIO_NUM_34   ///< Fin de carrera ABIERTO  (entrada sin pull interno)
#define PIN_MOTOR_A    GPIO_NUM_13   ///< Relé motor ABRIR
#define PIN_MOTOR_C    GPIO_NUM_12   ///< Relé motor CERRAR (Pin strapping: cuidar estados al boot)
#define PIN_LAMP       GPIO_NUM_2    ///< Lámpara/LED de estado (Pin strapping: cuidar estados al boot)

// Lógica de entradas (NC con pull-up externo → activo=0)
#define LM_ACTIVO      0
#define LM_NOACTIVO    (!LM_ACTIVO)

// Tiempos (ms)
#define T_OPEN_MS      15000         ///< Timeout máx. de apertura
#define T_CLOSE_MS     15000         ///< Timeout máx. de cierre
#define DEBOUNCE_MS    20            ///< Antirrebote por software
#define PUB_PERIOD_MS  30000         ///< Periodo de telemetría MQTT

// ===================== Portal WiFi AP/STA + MQTT (sin defaults) =====================
// Modo AP de configuración; activa portal en 192.168.4.1
...
// Modo de arranque
#define BOOTMODE_CONFIG_AP  0   ///< Arranca AP + portal de configuración
#define BOOTMODE_STA_ONLY   1   ///< Arranca solo en STA (sin AP)
...
static char g_wifi_ssid_cfg[33] = "";   ///< SSID guardado en NVS
static char g_wifi_pass_cfg[65] = "";   ///< PASS guardado en NVS
static bool g_have_creds = false;       ///< Señal de tener SSID escrito
static bool g_wifi_connected = false;   ///< Señal de conexión STA activa
static char g_sta_ip[16] = "0.0.0.0";   ///< IP STA actual
...
static bool g_ap_enabled = false;       ///< AP encendido (en modo APSTA)

// MQTT (variables configurables, SIN defaults)
static char g_mqtt_uri[128]    = "";   ///< p.ej. mqtt://host:1883
static char g_topic_cmd[96]    = "";   ///< suscripción (comandos)
static char g_topic_status[96] = "";   ///< publicación estado
static char g_topic_tele[96]   = "";   ///< publicación telemetría

// ------------------------------- FSM / COLA -----------------------------------
/** Comandos que puede recibir el sistema por MQTT. */
typedef enum {
    CMD_NONE = 0,
    CMD_OPEN,
    CMD_CLOSE,
...
static QueueHandle_t q_cmd;         ///< Cola de comandos entrantes (desde MQTT)
static uint64_t g_last_pub_us = 0;  ///< Timestamp última telemetría
static volatile int g_estado = ESTADO_INICIAL; ///< Estado actual de la FSM
static int g_estado_prev = -1;                 ///< Último estado publicado
static int g_motorA = 0, g_motorC = 0;        ///< Flags salidas motor (abrir/cerrar)
...
// Timeout conexión (30s sin IP ⇒ reboot a AP)
...
// ============================================================================
//                        URL decode helpers (form-urlencoded)
// ============================================================================
/**
 * @brief Convierte un carácter hexadecimal a valor (0-15). Retorna -1 si inválido.
 */
 ...
 *
 * @param s Cadena mutable a decodificar (se modifica in-place).
 */
static void url_decode_inplace(char *s) {
    ...
    while (*r) {
        ...
        if (ch == '+') { *w++ = ' '; r++; }
            ...
            else { *w++ = *r++; } // Secuencia inválida → copia literal
        } else {
            *w++ = *r++;
        }
    }
...
// ============================================================================
//                                   NVS
// Persistencia de: modo boot, WiFi (SSID/PASS), MQTT (URI/tópicos)
// ============================================================================
/** @brief Guarda modo de arranque en NVS. */
...
/** @brief Carga modo de arranque desde NVS (default: AP). */
...
/** @brief Guarda SSID/PASS en NVS (no guarda en driver WiFi). */
...
/** @brief Carga SSID/PASS desde NVS a buffers globales. */
...
/** @brief Guarda URI MQTT y tópicos en NVS. */
...
/** @brief Carga URI MQTT y tópicos desde NVS. */
...
/**
 * @brief Borra credenciales (WiFi/MQTT) y vuelve a modo AP de configuración.
 *        Limpia variables globales y restablece bootmode en NVS.
 */
...
// ============================================================================
//                 Aplicación de parámetros desde pares clave-valor
//              (reutilizable tanto para GET cortos como para POST)
// ============================================================================
/**
 * @brief Aplica parámetros WiFi desde cadena "key=value&...".
 * @param kv Cadena form-urlencoded (puede venir de query o body).
 *
 * Efectos:
 *  - Guarda SSID/PASS en NVS.
 *  - Reconfigura el driver STA y fuerza reconexión.
 *  - Activa temporizador de 30s para obtener IP.
 *  - Mantiene AP activo hasta obtener IP (BOOTMODE_CONFIG_AP).
 */
static void apply_wifi_from_kvstring(const char *kv) {
    ...
    if (httpd_query_key_value(kv, "ssid", ssid, sizeof(ssid)) == ESP_OK) {
        ...
        if (ssid[0] == '\0') {
            ...
            // Mantener AP hasta obtener IP para que el usuario vea el portal.
...
/**
 * @brief Aplica parámetros MQTT desde cadena "key=value&...".
 * @param kv Cadena form-urlencoded.
 *
 * Efectos:
 *  - Guarda URI y tópicos en NVS.
 *  - Reinicia cliente MQTT (sin tocar WiFi).
 */
...
// ============================================================================
//                          HTTP portal (GET/POST)
//  - Form GET: soporta acciones cortas (wipe, act=wifi/mqtt con query breve).
//  - Form POST: maneja payload largo evitando 431 Header too large.
// ============================================================================
/** @brief Handler GET del portal raíz. Renderiza HTML + procesa acciones cortas. */
static esp_err_t root_get_handler(httpd_req_t *req) {
    ...
    if (qlen > 0 && qlen < (int)sizeof(query)) {
        if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
            ...
            // 1) Wipe: borra credenciales y reinicia en modo AP.
            char wipe[8]={0};
            if (httpd_query_key_value(query, "wipe", wipe, sizeof(wipe)) == ESP_OK && !strcmp(wipe,"1")) {
                erase_all_creds_nvs();
                esp_wifi_stop();
                esp_wifi_restore();          // Limpia config del driver WiFi (RAM)
                save_boot_mode_to_nvs(BOOTMODE_CONFIG_AP);
            ...
            // 2) Acciones: act=wifi | act=mqtt (también por GET si son cortas)
            char act[16]={0};
...
/** @brief Handler POST del portal raíz. Lee body x-www-form-urlencoded y aplica cambios. */
static esp_err_t root_post_handler(httpd_req_t *req) {
    ...
    // Determinar acción (act=wifi | act=mqtt)
    char act[16]={0};
    if (httpd_query_key_value(body, "act", act, sizeof(act)) == ESP_OK) {
        if (!strcmp(act, "wifi"))      apply_wifi_from_kvstring(body);
        else if (!strcmp(act, "mqtt")) apply_mqtt_from_kvstring(body);
    }
...
/** @brief Inicia servidor HTTP y registra handlers GET/POST de "/". */
static httpd_handle_t start_webserver(void) {
...
// ============================================================================
//                            Eventos WiFi/IP + Timeout
//  - Maneja arranque STA, reconexión, obtención de IP y transición AP→STA.
//  - Tarea de watchdog de conexión: 30s sin IP ⇒ reinicio a AP.
// ============================================================================
/**
 * @brief Manejador de eventos WiFi/IP: reconexión, obtención de IP, etc.
 */
static void wifi_event_handler(void* arg, esp_event_base_t base, int32_t id, void* data) {
    if (base == WIFI_EVENT) {
        ...
        // Al obtener IP, apaga AP y queda solo en STA. Persiste modo.
...
/**
 * @brief Tarea watchdog de conexión. Si pasan 30s sin IP, reinicia y vuelve a modo AP.
 */
...
// ============================================================================
//                              Inicialización WiFi
//  - Decide AP/STA según NVS y credenciales.
//  - Lanza servidor HTTP y la tarea de timeout de conexión.
// ============================================================================
/**
 * @brief Inicializa netif/eventos, carga NVS, configura AP/STA, y levanta portal.
 */
static void wifi_init_sta(void) {
    ...
    // Crea interfaces AP/STA según modo
    esp_netif_create_default_wifi_sta();
    if (g_boot_mode == BOOTMODE_CONFIG_AP || !g_have_creds) esp_netif_create_default_wifi_ap();

    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    ESP_ERROR_CHECK(esp_wifi_init(&cfg));
    // Evita que el driver WiFi persista su propia config en flash (usamos NVS propio)
    ...
    if (g_boot_mode == BOOTMODE_CONFIG_AP || !g_have_creds) {
        // --- Modo APSTA: expone portal en 192.168.4.1 mientras no haya IP STA ---
        ...
        // --- Modo STA puro: intenta conectar con credenciales de NVS ---
    ...
    // Portal HTTP + watchdog de conexión
    g_httpd = start_webserver();
    xTaskCreate(connect_timeout_task, "connect_timeout_task", 4096, NULL, 5, NULL);
}

// ============================================================================
//                         UTILIDADES DE I/O Y TELEMETRÍA
// ============================================================================
/**
 * @brief Antirrebote por software: retorna nivel estable tras 'ms' milisegundos.
 */
...
/** @brief Actualiza g_lsa/g_lsc aplicando antirrebote y mapeando a LM_ACTIVO. */
...
/** @brief Convierte código de estado en cadena legible. */
...
// ---- Acciones sobre relés (interlock garantizado) ----
...
/**
 * @brief Publica un JSON a un tópico MQTT (QoS1, retain=1).
 * @param topic        Tópico destino (debe estar configurado).
 * @param include_mot  Incluye flags de motor en el JSON.
 * @param include_err  Incluye g_error_code en el JSON.
 */
...
/** @brief Publica estado sólo si hubo cambio (y traza en log). */
...
/** @brief Telemetría periódica a g_topic_tele. */
static inline void tick_telemetria(void) {
...
/** @brief Extrae un comando de la cola sin bloquear. */
static inline bool fetch_cmd(gate_cmd_t *out_cmd) { return xQueueReceive(q_cmd, out_cmd, 0) == pdTRUE; }

// ============================================================================
//                               MQTT dinámico
//  - Suscripción al tópico de comandos si está configurado.
//  - Parseo de JSON {"cmd":"..."} y encolado hacia la FSM.
// ============================================================================
/** @brief Parsea payload JSON de comando a enumeración gate_cmd_t. */
...
/** @brief Handler de eventos MQTT: conexión y recepción de datos. */
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch (event_id) {
        ...
        case MQTT_EVENT_DATA: {
            // Copiar payload para parsear con terminador
...
/** @brief Inicializa cliente MQTT si existe un URI configurado. */
static void mqtt_init(void) {
    if (!g_mqtt_uri[0]) {
...
/** @brief Reinicia cliente MQTT (usar tras cambiar URI/tópicos). */
static void mqtt_restart(void) {
...
// ============================================================================
//                               FSM: BUCLES DE ESTADO
//  Patrón: leer sensores → verificar seguridad → decidir transición por sensores
//          → procesar comandos → telemetría → publicar si cambió → pequeño delay.
// ============================================================================
...
/** @brief Estado de arranque: clasifica posición según LS. */
...
// ============================================================================
/**
 * @brief Tarea principal de FSM. Ejecuta el bucle del estado actual,
 *        y adopta el estado retornado por cada bucle.
 */
...
// ============================================================================
/**
 * @brief Configura GPIOs de entradas/salidas (sin pulls en 34/35) y deja HW en seguro.
 */
...
/**
 * @brief Punto de entrada: NVS, GPIO, WiFi (AP/STA + portal), cola FSM y MQTT (si hay URI).
 */
void app_main(void) {
    // NVS listo (maneja actualización de versión/espacio)
    ...
    gpio_init_all();
    wifi_init_sta();      // decide AP/STA, portal HTTP y watchdog

    q_cmd = xQueueCreate(16, sizeof(gate_cmd_t));
    if (g_mqtt_uri[0]) mqtt_init();   // MQTT sólo si hay broker configurado

    // FSM principal (prioridad alta)
